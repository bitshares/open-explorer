    ...
    function blocksCtrl($scope, $filter, $routeParams, $http, appConfig) {
        ...
        $http.get(appConfig.urls.python_backend + "/get_block?block_num=" + name)
            .then(function(response) {
                ...
                $scope.data = { transactions: response.data.transactions, block_num: name, previous: response.data.previous, timestamp: response.data.timestamp, witness: response.data.witness, witness_signature: response.data.witness_signature, transaction_merkle_root: response.data.transaction_merkle_root, transactions_count: response.data.transactions.length, operations_count: operations_count};
            });
    }
